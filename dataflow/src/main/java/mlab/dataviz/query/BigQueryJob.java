...
public class BigQueryJob {
	...
	/**
	 * Building retry settings to prevent an HTTP timeout for bigquery jobs.
	 * Extending the timeout on the bigquery rpc isn't enough since it goes over
	 * HTTP and that connection can timeout too.
	 * 
	 * @return
	 */
	...
	/**
	 * @constructor Creates a new bigquery querier. Can be used to run legacy and
	 *              standard sql queries against bigquery.
	 * @throws IOException
	 */
	public BigQueryJob() throws IOException {
		...
		transportOptions = transportOptions.toBuilder().setConnectTimeout(120000).setReadTimeout(120000).build();

		BigQueryOptions bigqueryOptions = BigQueryOptions.newBuilder().setRetrySettings(retrySettings())
	...
	/**
	 * Executes a given query synchronously. Returns the rows that resulted from
	 * that query.
	 *
	 ...
	 */
	public Iterable<FieldValueList> executeQuery(String querySql) throws IOException, InterruptedException {
		QueryJobConfiguration queryConfig = QueryJobConfiguration.of(querySql);
		Job job = bigquery.create(JobInfo.of(queryConfig));
		return job.getQueryResults(maxWaitTime(60000L), pageSize(1000L)).getValues();
	}

	/**
	 * Executes a query when the known response is a single string value. By default
	 * uses legacy SQL.
	 * 
	 * @param querySql
	 ...
	 * @throws InterruptedException
	 */
	public String executeQueryForValue(String querySql)
			throws IOException, InterruptedException, GoogleJsonResponseException, BigQueryException {
	...
	public String executeQueryForValue(String querySql, boolean legacySql) throws IOException, InterruptedException, GoogleJsonResponseException, BigQueryException {
		...
		try {
			...
			Job job = bigquery.create(JobInfo.of(queryConfig));
			TableResult queryResults = job.getQueryResults(maxWaitTime(300000L), pageSize(1000L));
			
			String val = null;
			for (FieldValueList row : queryResults.getValues()) {
